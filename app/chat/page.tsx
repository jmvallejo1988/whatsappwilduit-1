...
export default function ChatListPage() {
  ...
  const handleNewChat = () => {
    ...
    if (clean.length >= 10) router.push(`/chat/${clean}`);
  };

  const avatarColor = (name: string) => {
    const colors = ['#6b7c85', '#00a884', '#128C7E', '#075E54', '#34B7F1'];
    ...
    <div className="h-screen bg-[#111b21] flex flex-col max-w-md mx-auto relative">
      ...
      <div className="bg-[#202c33] px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchConversations} className="text-[#8696a0] hover:text-white transition-colors p-1" title="Actualizar">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
          </button>
          <button onClick={handleLogout} className="text-[#8696a0] hover:text-white transition-colors p-1" title="Cerrar sesión">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="px-3 py-2 bg-[#111b21]">
        <div className="flex gap-2">
          <input
            ...
            placeholder="Número nuevo (ej: 5491112345678)"
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-56 px-8 text-center">
            ...
            <p className="text-[#8696a0] text-xs mt-1">Los mensajes de WhatsApp aparecerán aquí</p>
          </div>
        ) : (
          <div>
            {conversations.map((conv) => (
              <Link
                ...
                className="flex items-center px-4 py-3 hover:bg-[#2a3942] active:bg-[#2a3942] transition-colors border-b border-[#2a3942]/50"
              >
                <div
      ...
      <button
        ...
        className="absolute bottom-6 right-4 w-14 h-14 bg-[#00a884] rounded-full flex items-center justify-center shadow-xl hover:bg-[#06cf9c] transition-colors"
      >
